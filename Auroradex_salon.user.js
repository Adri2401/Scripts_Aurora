...
(function () {
  ...
  function stop(reason) {
    ...
    const tipo = /^✅/.test(reason) ? (/Cupo/.test(reason) ? 'exito' : 'fin') : /Energía en|sin energía/i.test(reason) ? 'energia' : 'error';
    const vales = ses.vales != null && ses.vales0 != null ? ses.vales - ses.vales0 : null;
    kAviso({
      tipo, app: 'Salón Malvalona', icono: tipo === 'exito' || tipo === 'fin' ? '🃏' : null,
      titulo: tipo === 'exito' ? '¡Respiros de hoy comprados!' : tipo === 'fin' ? 'Partidas terminadas' : tipo === 'energia' ? 'Parado por la energía' : 'El Salón se ha parado',
      texto: tipo === 'exito' || tipo === 'fin' ? null : reason,
