...
(function (raiz) {
  ...
  async function jugar(opc = {}) {
    ...
    try {
      while (!parando) {
        ...
        if (!est) {
          if (++sinMesa > 6) { log('No consigo leer el tablero. Paro.'); return; }
          await dormir(o.retardo);
          continue;
        }
        sinMesa = 0;
        if (est.pendientes.length) {
          if (++esperas * 150 > 6000) { log('Una casilla no termina de revelarse. Paro.'); return; }
          await dormir(150);
          continue;
        }
        esperas = 0;
        ...
        if (!d) {
          ...
          log('No quedan casillas útiles y no hay botón de plantarse. Paro.');
          return;
        }
        ...
        if (!est.clicable(d.idx)) { log(`Casilla ${d.idx + 1} no clicable (¿modo marcar?). Paro.`); return; }
        ...
        if (est2 && est2.conocidas.join(',') === firma && !boton(/mesa nueva/i)) {
          if (++atascos >= 3) { log('El clic no cambia el tablero. Paro.'); return; }
        } else atascos = 0;
    ...
    try {
      ...
      while (!cableParar) {
        ...
        if (cambio) {
          ...
          cableMsg('Vender no surte efecto. Paro.'); break;
        }
      }
      cableMsg(cableParar
    ...
    try {
      if (f.fase === 'comprar') {
        ...
        if (enMercado()) {
          ...
          if (t) t.click();
          if (n > 10) { finFlujo(); cableMsg('No encuentro el Cable Unión en el mercado.'); }
          return;
        }
  ...
  function tickFlujo() {
    ...
    if (Date.now() - f.ts > 300000) { finFlujo(); cableMsg('Flujo automático cancelado (tiempo agotado).'); return; }
    pasoFlujo(f);
  }
