...
(function () {
  ...
  function readState() {
    ...
    if (sec) {
      ...
      st.name = alt || grande || '?';
      st.sprite = img ? img.getAttribute('src') : null;
  ...
  function newEncounter(st) {
    ...
    if (st.shiny) {
      ...
      kAviso({ tipo: 'shiny', app: 'Safari', titulo: `¡${st.name} shiny!`, sprite: st.sprite, lineas: [st.level || null, 'Lo intento con todo, sin mirar lo que cuestan las Balls.'] });
  ...
  function stop(reason) {
    ...
    if (!iba || !reason || /a mano|Fuera del Safari|en la entrada/i.test(reason)) return;
    const tipo = /^✅|Se acabó/.test(reason) ? 'exito' : /Sin Balls/i.test(reason) ? 'aviso' : 'error';
    const atrapados = ses.caught != null && ses.caught0 != null ? ses.caught - ses.caught0 : null;
    kAviso({
      tipo, app: 'Safari', icono: tipo === 'exito' ? '🌿' : null,
      titulo: tipo === 'exito' ? 'Visita al Safari terminada' : tipo === 'aviso' ? 'Te has quedado sin Balls' : 'El Safari se ha parado',
      texto: tipo === 'error' ? reason : null,
      lineas: [`🔍 ${ses.enc} encuentros${atrapados != null ? ` · 🎯 ${atrapados} atrapados` : ''} · ⏱ ${kTime((ses.t1 || Date.now()) - ses.t0)}`, ses.shinies && ses.shinies.length ? `✨ Shiny: ${ses.shinies.join(', ')}` : null],
    });
  }
  ...
  function buildPanel() {
    kStyle('ax-safari-kit', UP, '#7FA83A');
